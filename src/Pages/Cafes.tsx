...
const Cafes: React.FC<CafesProps> = ({ language }) => {
  ...
  const companyCafes = cafes[language].filter((cafe: Cafe) => cafe.elements.country.value === 'USA');
  const partnerCafes = cafes[language].filter((cafe: Cafe) => cafe.elements.country.value !== 'USA');
  ...
  const partnerCafeModels = partnerCafes.map((cafe : Cafe) => createCafeModel(cafe));
